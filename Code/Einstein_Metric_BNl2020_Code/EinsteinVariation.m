...
MetricFromDS::usage = "MetricFromDS[ds,X] takes a line element and generates
...
MetricFromDS[ds_,X_,simpopt_:{TimeConstraint->10}] := Module[{dX,gll, iindex,jindex,dsi,Dim},
                    Dim = Length[X];
 		    gll = Table[0,{iXX,1,Dim},{jXX,1,Dim}]; 
                    dX = Table[ToExpression["d"<>ToString[X[[jXX]]]],{jXX,1,Dim}];
                    dsi = Expand[ds];
                    For[iindex = 1, iindex <= Dim, iindex = iindex+1,
                        gll[[iindex,iindex]] = Simplify[Coefficient[dsi, dX[[iindex]],2]];
                        For[jindex = iindex+1, jindex <= Dim, jindex = jindex+1,
                            gll[[iindex,jindex]] = (1/2)  Simplify[Coefficient[dsi, dX[[iindex]] dX[[jindex]],1]];
...
LineltFromMetric[gll_, X_] := Module[{dX, ds, Dim, iindex, jindex},
                    Dim = Length[X];
                    dX = Table[ToExpression["d" <> ToString[X[[jXX]]]], {jXX, 1, Dim}];
                    ds = Sum[Sum[If[jXX ==  kXX, Simplify[gll[[jXX, kXX]]] dX[[jXX]] dX[[kXX]],2 Simplify[gll[[jXX, kXX]]] dX[[jXX]] dX[[kXX]]], 
                    {kXX, jXX, Dim}], {jXX, 1, Dim}];
                    Return[ds];
                   ]                    
                    
GetCClll[gll_,X_,simpopt_:{TimeConstraint->10}] := Module[{CClllout, Dim},
                    Dim = Length[X];
...
GetCCull[gll_,X_,simpopt_:{TimeConstraint->10}] := Module[{CCullout, guu, CClllin,Dim},
                    Dim = Length[X];
...
GetRiemann[gll_,X_, simpopt_:{TimeConstraint->10}] := Module[{Rulllout,CCull,Dim},
                    Dim = Length[X];
...
GetRicci[gll_,X_, simpopt_:{TimeConstraint->10}] := Module[{Rulll, Rll,Dim},
                    Dim = Length[X];
...
GetRicciS[gll_,X_, simpopt_:{TimeConstraint->10}] := Module[{Rs, guu,Dim},
                    Dim = Length[X];
...
GetKerrgll[M_,a_,r_,T_] := FullSimplify[{{-1 + 2 M r/(a^2 Cos[T]^2 + r^2),0,0,-2 a M r Sin[T]^2/(r^2 + a^2 Cos[T]^2)},
			      {0, (a^2 Cos[T]^2 + r^2)/(a^2 - 2 M r + r^2),0,0},
			      {0,0,a^2 Cos[T]^2 + r^2,0},
			      {-2 a M r Sin[T]^2/(r^2 + a^2 Cos[T]^2),0,0,(Sin[T]^2 ( (a^2 + r^2)^2 - a^2 (a^2 - 2 M r + r^2) Sin[T]^2))/(a^2 Cos[T]^2 + r^2)}}]
